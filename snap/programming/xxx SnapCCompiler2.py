...
# TODO this is still useful for internal or wrapper generation...
...
def build(ENV):
	...
	class SnapCCompiler(SnapNode):
		...
		class indent_token:
			...
			def get(self, MSG):
				"()->str"
				t = self.__snap_data__['indent_token']
				if t is None:
					t = '\t'
				return t
		...
		class is_headerfile:
			...
			def get(self, MSG):
				"()->bool"
				return bool(self.__snap_data__['is_headerfile'])
		...
		def compiler_push_fblock(self, NODE, BLOCK, EXIT):
			# https://github.com/python/cpython/blob/d93605de7232da5e6a182fd1d5c220639e900159/Python/compile.c#L1629

			frame_blocks = self['__frame_blocks__']
			assert isinstance(frame_blocks, list), 'frame_blocks None?'
			assert len(frame_blocks) < 10000, 'too many statically nested blocks' # TODO proper limit
			frame_blocks.append({
				'type':NODE['__type__'],
				'node':NODE,
				'block':BLOCK,
		...
		def compiler_pop_fblock(self, NODE, BLOCK):
			frame_blocks = self['__frame_blocks__']
			assert isinstance(frame_blocks, list), 'frame_blocks None?'
			assert len(frame_blocks) > 0
			assert frame_blocks[-1]['node'] is NODE and frame_blocks[-1]['block'] is BLOCK
			frame_blocks.pop()

		def compiler_use_next_block(self, BLOCK):
			'' # TODO
		...
		def ADDOP(self, OP):
			# https://github.com/python/cpython/blob/d93605de7232da5e6a182fd1d5c220639e900159/Python/compile.c#L1155
		...
		def compiler_addop_j(self, OP, BLOCK, ABSOLUTE):
			# https://github.com/python/cpython/blob/d93605de7232da5e6a182fd1d5c220639e900159/Python/compile.c#L1508
		...
		def ADDOP_JREL(self, OP, BLOCK):
			return self.compiler_addop_j(OP, BLOCK, False)

		def ADDOP_JABS(self, OP, BLOCK):
			return self.compiler_addop_j(OP, BLOCK, True)

		def visit_for(self, NODE):
			# https://github.com/python/cpython/blob/d93605de7232da5e6a182fd1d5c220639e900159/Python/compile.c#L2651

			start,end,cleanup = {},{},{}
			self.compiler_push_fblock(NODE, start, end)

			self.visit(NODE['in'])
			self.ADDOP('GET_ITER') # STACK[-1] = iter(STACK[-1])
			self.compiler_use_next_block(start)
			self.ADDOP_JREL('FOR_ITER', cleanup)
			self.visit(NODE['target'])
			for stmt in NODE['body']:
				self.visit(stmt)
			self.ADDOP_JABS('JUMP_ABSOLUTE', start)
			self.compiler_use_next_block(cleanup)

			self.compiler_pop_fblock(NODE, start)

			for stmt in NODE['else']:
				self.visit(stmt)
			self.compiler_use_next_block(end)
		...
		def visit(self, NODE):
			...
			# visit method for each one, so that they can be overridden if a language needs different behaviour...?

			visit = getattr(self, 'visit_' + NODE['__type__'], None)
			if visit is not None:
				visit(NODE)
			else:
				ENV.snap_warning('missed', NODE['__type__'])
