...
def build(ENV):
	...
	class SnapEBNFDecoder(ParseqDecoder):
					...
					if not rule_type:
						...
						if rule_type == 'ITEM':
							...
							if name.startswith('"') and name.endswith('"'):
								...
								ENV.snap_debug('name okay (no quotes)?', repr(name))
	...
	ENV.SnapEBNFDecoder = SnapEBNFDecoder
...
def main(ENV):
	...
	decoder = SnapEBNFDecoder()
	
	with open(GRAMMAR_FILE, 'r') as openfile:
		single_input = decoder.decode(openfile.read(),
