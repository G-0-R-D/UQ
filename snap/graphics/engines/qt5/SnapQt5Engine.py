...
def build(ENV):
	...
	class SnapQt5Engine(SnapEngine):
		...
		def __init__(self, **SETTINGS):
			...
			ENV.__build__('snap.graphics.engines.qt5.paint.SnapQt5Color')
			self.Color = self.SnapQt5Color

			ENV.__build__('snap.graphics.engines.qt5.paint.SnapQt5Gradient')
			self.Gradient = self.SnapQt5Gradient

			ENV.__build__('snap.graphics.engines.qt5.paint.SnapQt5Texture')
			self.Texture = self.SnapQt5Texture

			ENV.__build__('snap.graphics.engines.qt5.shape.SnapQt5Spline')
			self.Spline = self.SnapQt5Spline

			ENV.__build__('snap.graphics.engines.qt5.shape.SnapQt5Mesh')
			self.Mesh = self.SnapQt5Mesh

			ENV.__build__('snap.graphics.engines.qt5.shape.SnapQt5Image')
			self.Image = self.SnapQt5Image

			#ENV.__build__('snap.graphics.engines.qt5.shape.text.SnapQt5TextMetrics')
			#self.TextMetrics = self.SnapQt5TextMetrics

			ENV.__build__('snap.graphics.engines.qt5.shape.SnapQt5Text')
			self.Text = self.SnapQt5Text

			ENV.__build__('snap.graphics.engines.qt5.SnapQt5Context')
