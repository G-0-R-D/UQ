...
# since my graphic library is designed for a data-driven approach, rather than realtime drawing on-demand,
# I thought I would also make an example showing a more optimal approach for how I would do this in snap...
...
def build(ENV):
	...
	class Boid(SnapContainer):
		...
		# pre-making the boid shape will go much faster, and they can all share the same one
		SHAPE = ENV.GRAPHICS.Spline(description=['S', 0,-radius*2, -radius,radius*2, radius,radius*2, 'C'])
		...
		class shape:
			...
			def get(self, MSG):
				"()->SnapSpline"
				return Boid.SHAPE # could give a unique shape
		...
		class color:
			...
			def get(self, MSG):
				"()->SnapColor"
				return Boid.COLOR # could give unique color...
		...
		class render_matrix:
			...
			def get(self, MSG):
				...
				# TODO this could be more optimized if location and rotation were being calculated on-demand,
				# but I can't be bothered to remap the backend right now...
				...
				M.translate(self.location.x, self.location.y)
		...
		def draw(self, CTX):
			CTX.cmd_stroke_spline(self['color'], self['shape'])

		def lookup(self, CTX):
			'' # TODO make them interactive (hover changes color or something, or make them click and draggable...)
